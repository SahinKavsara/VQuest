...
## 7. Odadan Katılımcı Çıkarma (Kick) Arayüzü
- **API Endpoint:** `DELETE /api/rooms/:roomId/participants/:userId`
- **Görev:** Oda kurucusunun katılımcı listesinden bir kullanıcıyı seçerek odadan çıkarabileceği arayüz akışının tasarımı ve implementasyonu.
- **UI Bileşenleri:**
  - Katılımcı listesinde her kullanıcının yanında "Çıkar" (Kick) ikonu/butonu (yalnızca host'a görünür)
  - Onay dialog'u ("Bu kullanıcıyı odadan çıkarmak istediğinize emin misiniz?")
  - İşlem sırasında loading indicator
- **Kullanıcı Deneyimi:**
  - Yalnızca oda kurucusuna "Çıkar" butonu görünür (yetki kontrolü frontend'de de yapılır)
  - Destructive action için kırmızı renk ve uyarı ikonu kullanılır
  - Onay sonrasında katılımcı listeden animasyonlu şekilde kaldırılır
  - Hata durumunda `Alert.alert` ile kullanıcıya açıklayıcı mesaj gösterilir
- **Akış Adımları:**
  1. Host, katılımcı yanındaki "Çıkar" ikonuna basar
  2. Onay dialog'u gösterilir
  3. Onaylandığında `DELETE` isteği gönderilir
  4. Başarılı işlem sonrası katılımcı listeden kaldırılır
- **Teknik Detaylar:**
  - `api.js` instance'ı üzerinden `axios.delete('/api/rooms/:roomId/participants/:userId')` çağrısı
  - Zustand store: `useRoomStore` içinde `kickParticipant` action'ı; `participants[]` state'inden ilgili kullanıcı çıkarılır
  - Socket.io client: `participant:left` eventi dinlenerek diğer katılımcıların listesi de güncellenir
  - Hata anında `Alert.alert('Çıkarma Başarısız', error.response?.data?.message || 'Kullanıcı çıkarılamadı.')` gösterilir

## 8. Oda Kapatma/Sonlandırma Arayüzü
